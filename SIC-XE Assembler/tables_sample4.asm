************************SYMBOL TABLE*************************

:-	name:undefined	|address:0	|relative:00000 
0:-	name:	|address:0	|relative:00000 
COUNT:-	name:COUNT	|address:0001A	|relative:00001 
FIRST:-	name:FIRST	|address:00000	|relative:00001 
LOOP:-	name:LOOP	|address:0000A	|relative:00001 
TABLE:-	name:TABLE	|address:0001D	|relative:00001 
TABLE2:-	name:TABLE2	|address:0178D	|relative:00001 
TOTAL:-	name:TOTAL	|address:02EFD	|relative:00001 

---------------------------------LITERAL TABLE---------------------------------


---------------------------------BLOCK TABLE---------------------------------

DEFAULT:-	value:DEFAULT	|address:00000 

